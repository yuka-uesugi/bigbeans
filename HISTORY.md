...
- **第2段階（メール通知）コード実装完了（未デプロイ）**: Gmail直送＋Vercel APIルート方式で実装済み。型チェック・ビルド成功。
  - **動かすにはオーナーの設定作業が必要**: ①Gmailアプリパスワード ②Firebaseサービスアカウント JSON を Vercel と `.env.local` に貼り付け。手順は `docs/メール通知_設定手順.md`。
  - 設定後にローカルでテスト → 問題なければデプロイ。
- 第2段階の詳細は「8. 通知機能」と `docs/通知機能_仕様書.md`、設定は `docs/メール通知_設定手順.md` を参照。
- **コミット予定**: 第2段階のコード（route.ts / firebaseAdmin.ts / notifications.ts / profile / 各docs / package.json）。`.env.local` はgit対象外。
