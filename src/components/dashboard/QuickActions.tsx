...
const actions = [
  ...
  { icon: "📊", label: "アンケート", color: "from-sky-400 to-sky-500",            href: "/dashboard/surveys" },
  { icon: "📒", label: "議事録",     color: "from-indigo-400 to-indigo-500",      href: "/dashboard#minutes" },
      ...
      <div className="grid grid-cols-2 gap-3">
        {actions.map((action) => (
          <Link
            key={action.label}
            href={action.href}
            className="group flex flex-col items-center gap-2 p-4 rounded-xl bg-ag-gray-50 hover:bg-ag-gray-100 border border-transparent hover:border-ag-gray-200/60 transition-all duration-200"
          >
